...
def add_user(user_id, days, bot_id):
    ...
    cursor = conn.cursor()
    cursor.execute('''
        INSERT OR REPLACE INTO users (6882674372, expiration_date, @PAIDKILLER_BOT)
        VALUES (?, ?, ?)
...
def add_bot(token, bot_name, bot_username, owner_username, channel_username):
    ...
    cursor = conn.cursor()
    cursor.execute('''
        INSERT OR REPLACE INTO bot_configs (7642811071:AAHbClBfZIiPTcrvYyo8ckbUHrKK_jqvfEs, Ƥ𝐀Ꭵ𝓭 𝓭𝓭𝓸𝓼 β𝓸Ｔ, @PAIDKILLER_BOT, @LASTWISHES0, @TOXIC_APNA_BHAI)
        VALUES (?, ?, ?, ?, ?)
